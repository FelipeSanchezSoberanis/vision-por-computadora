...
def band_pass_filter(
    ...
    for x in range(height):
        for y in range(width):
            dft_shift[x][y] *= comparator(x_center, y_center, x, y, r1, order) * (
...
def main() -> None:
    ...
    image_ideal: np.ndarray = band_pass_filter(
        image_original, 30, 100, filter_comparator_ideal
    )
    image_butterworth: np.ndarray = band_pass_filter(
        image_original, 30, 100, filter_comparator_butterworth, order=10
    )
    image_gaussian: np.ndarray = band_pass_filter(
        image_original, 30, 100, filter_comparator_gaussian
    )
